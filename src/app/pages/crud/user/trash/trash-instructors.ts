...
@Component({
    selector: 'app-user-trash-instructors',
    standalone: true,
    ...
    templateUrl: './trash-instructors.html',
    providers: [MessageService, ConfirmationService]
})
export class UserTrashInstructors implements OnInit {
    ...
    loadUsers(event?: any) {
        ...
        this.userService.getDeletedInstructors(page, pageSize).subscribe({
            next: (response) => {
                if (response && response.data) {
                    // Handle both paged response (data.items) and flat array response (data)
                    const isArray = Array.isArray(response.data);
                    this.users.set(isArray ? response.data : response.data.items || []);
                    this.totalRecords = isArray ? response.data.length : response.data.totalCount || 0;
                }
                this.loading = false;
            },
            error: (err) => {
